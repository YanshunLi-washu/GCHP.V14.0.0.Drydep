...
! !INTERFACE:
!
  FUNCTION AERO_SFCRSII( K, II, PRESS, TEMP, USTAR, RHB, W10, VTSout, &
                         Input_Opt, State_Chm ) &
       RESULT(RS)
!
! !USES:
...
!
    ...
    IF ((SpcInfo%Name .EQ. 'NIT') .OR. (SpcInfo%Name .EQ. 'NH4') &
        ...
        ELSE IF ((RHBL .GE. 0.35) .AND. (RHBL .LE. 0.40)) THEN
            DIAM = DIAM + (DIAM * ((1.0 + 0.61 * 0.40 / (1.0 - 0.40)) &
      ** (1.0 / 3.0)) - DIAM) / (0.40 - 0.35) * (RHBL - 0.35)
...
!
  ...
  FUNCTION DUST_SFCRSII( K, II, PRESS, TEMP, USTAR, &
                         DIAM, DEN, VTSout ) &
       RESULT( RS )
!
! !INPUT PARAMETERS:
